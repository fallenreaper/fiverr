...
def rebuild_proxylist():
  ...
  if len(PROXY_LIST) < 4:
    ...
    os.system(f"curl 'https://api.proxyscrape.com/v2/?request=getproxies&protocol=http&timeout={MAX_PROXY_LATENCY}&country=all&ssl=all&anonymity=all&simplified=true' > http_proxies.txt")
    with open("http_proxies.txt") as fp:
      PROXY_LIST = set([x.strip() for x in fp.readlines()])
    print("Proxy List Rebuilt.")
    if DEBUG: print(PROXY_LIST)
...
def worker():
  ...
  while True:
    ...
    try:
      ...
      try:
        _cost = bs_data.find("div", attrs={"id": "srp-item-price-1"}).text.split(" ")[1]
        if DEBUG: print(f"{response.status_code}, {proxies['http']} | Writing {isbn} => {_cost}")
        __RESULTS.append( (isbn, _cost))
        __counter += 1
      except AttributeError as e:
        if DEBUG: print(f"ISBN FAILURE: {isbn}")
      ...
      if DEBUG: print("Possible Proxy dead. ", _p)
      remove_dead_proxy(_p)
      _p = get_next_proxy()
      proxies = {
        "http": _p,
      ...
      if DEBUG: print("Failed to get file. ", e)
      _p = get_next_proxy()
      proxies = {
        "http": _p,
        "https": _p
...
def main():
  ...
  for isbn in ISBN_LIST:
    q.put( (isbn, 0))
  if DEBUG: print(f"Size: {len(ISBN_LIST)}")
...
if __name__ == '__main__':
  ...
  parser.add_argument('-o', '--outfile', help=f'File output.  Default: {OUTPUTFILE}', default=OUTPUTFILE)
  parser.add_argument('input', help=f"ABS path to Input xlsx ISBN file. Default: '{ISBN_FILE}''", default=ISBN_FILE)
  parser.add_argument('--retries', help="Max Retries", type=int, default=MAX_RETRIES)
  parser.add_argument('-t', '--threads', help="Number of Threads", type=int, default=NUM_THREADS)
  parser.add_argument('--sleep', help=f"Sleep Timer.  Default: {SLEEP_TIMER} seconds", type=float, default=SLEEP_TIMER)
  parser.add_argument('--max-proxy-latency', help=f"Max Latency for Free Proxies.  Default: '{MAX_PROXY_LATENCY}'", type=int, default=MAX_PROXY_LATENCY)
